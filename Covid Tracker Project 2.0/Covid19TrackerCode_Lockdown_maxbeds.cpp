...
int main()
{
	long long S,I,R,r0,V,maxbeds;
	double RecoveryRate,MortalityRate;

    long long saved = 0, recovered = 0, death = 0;

	cin>>S>>I>>R>>r0>>RecoveryRate>>MortalityRate>>V>>maxbeds;
	...
	while(I>0){
        ...
        if(f == true || ((S-(I*r0)-V) <= 0)){
            ...
            if((original_i*RecoveryRate)>maxbeds){ 
                I -= maxbeds;
             }  
             else{
            ...
            if((original_i*RecoveryRate)>maxbeds){
            cout<<"Month "<<day<<" :   "<<S<<"      "<<original_i<<"      "<<maxbeds<<"       "<<(int)(original_i*MortalityRate)<<endl;
        }
        else{
